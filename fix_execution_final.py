...
code = re.sub(r'        \.select => \|s\| \{\s+if \(catalog\.get_table\(s\.table_name\)\) \|table\| \{\s+var base_executor: exec\.Executor = undefined;\s+var seq_exec: exec\.SeqScanExecutor = undefined;\s+var index_exec: exec\.IndexScanExecutor = undefined;\s+var filter_exec: exec\.FilterExecutor = undefined;[\s\S]*?base_executor = \.\{ \.seq_scan = &seq_exec \};\s+\}\s+', select_start + "\n", code)

# Remove the matching closing brace and else { return error.TableNotFound; }
# This is located exactly around line 430 in original, before the end of .select.
# We will just replace it with empty by matching the exact end of .select handling in original code:
#             } else {
#                 return error.TableNotFound;
#             }
#         },

# But wait, it's safer to just remove it at the end of .select:
code = code.replace("""                while (try final_executor.next()) |tuple| {
...
# Fix table references in join code
...
code = code.replace("exec.resolve_column(table.schema,", "exec.resolve_column(table_schema,")
code = code.replace("table.num_tuples.load(.monotonic)", "num_left_tuples")
code = code.replace("table.schema.len", "table_schema.len")
code = code.replace("table.schema)", "table_schema)")

# Re-apply join optimizations
