...
pub fn main(init: std.process.Init) !void {
    ...
    if (run_all or run_buffer) try benchmark_buffer_manager(allocator, io);
    if (run_all or run_btree) try benchmark_btree(allocator, io);
    if (run_all or run_table) try benchmark_table(allocator, io);
    if (run_all or run_parser) try benchmark_parser(allocator);
    if (run_all or run_execution) try benchmark_execution(allocator, io);
    if (run_all or run_transaction) try benchmark_transaction(allocator, io);
    if (run_all or run_eviction) try benchmark_eviction(allocator, io);
...
fn benchmark_secondary_index(allocator: std.mem.Allocator, io: std.Io) !void {
    ...
    for (0..num_records) |i| {
        const t1 = try table.serialize_tuple(allocator, &[_]ast.Value{
            .{ .int = @intCast(i) }, .{ .int = @intCast(i % 100) }
        });
        defer allocator.free(t1);
