...
pub const ClusterConfig = struct {
    ...
    pub fn deserialize(allocator: std.mem.Allocator, data: []const u8) !ClusterConfig {
        ...
        var old_members: ?[][]const u8 = null;
        var old_alloc_len: usize = 0;
        var new_members: ?[][]const u8 = null;
        var new_alloc_len: usize = 0;
        errdefer {
            if (old_members) |om| {
                for (om[0..old_alloc_len]) |m| allocator.free(m);
                allocator.free(om);
            }
            if (new_members) |nm| {
                for (nm[0..new_alloc_len]) |m| allocator.free(m);
                allocator.free(nm);
            }
        }

        const old_buf = try allocator.alloc([]const u8, old_len);
        old_members = old_buf;
        for (0..old_len) |i| {
            if (offset + 4 > data.len) return error.EndOfStream;
        ...
        if (new_len > 0) {
            const nm = try allocator.alloc([]const u8, new_len);
            new_members = nm;
            for (0..new_len) |i| {
                if (offset + 4 > data.len) return error.EndOfStream;
                const m_len = std.mem.readInt(u32, data[offset .. offset + 4][0..4], .little);
