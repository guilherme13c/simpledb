...
def start_node(port, seeds):
    ...
    # We use preexec_fn=os.setsid so we can kill the whole process group if needed
...
def send_query(port, query, expect_ok=True):
    try:
...
def find_leader():
    for port in [8081, 8082, 8083]:
        # Try a dummy write to see who accepts it
...
send_query(leader, "CREATE TABLE chaos_tbl (id INT, val VARCHAR)")

# Explicitly add nodes to Raft config to test Joint Consensus
...
def inserter():
    ...
    while running:
        ...
        success, resp = send_query(current_leader, query)
        if not success:
            time.sleep(1)
...
print("Chaos: Resuming all nodes")
...
def get_count(port):
    # we modified simpledb connection logic to explicitly ALLOW select queries on read-only replicas!
    success, resp = send_query(port, "SELECT count(*) FROM chaos_tbl", expect_ok=False)
    if success and "OK" in resp:
        try:
            lines = resp.split("\n")
